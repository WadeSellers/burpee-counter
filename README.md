...
- 50: roughly 2.5s bigger invert takeover, "Halfway" — then, when the shout-out pool has handles, a three-beat shout-out show (~5.6s): "SHOUT OUT" slams in with gold spark bursts, the screen flips white with the handle in a letter wave between stars, then flips back with the handle bouncing in gold over a random compliment line ("YOU SHOWED UP", "LEGEND", ...). Confetti on every beat; taps still count throughout.
